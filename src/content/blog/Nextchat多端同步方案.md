...
3.  **高级配置（推荐）**：
    ...
    *   **同步密码**：建议设置。设置后，数据在上传云端前会进行加密，即使他人拿到 Token 也无法解密你的聊天内容。
4.  **激活同步**：
    *   点击 **“立即同步”**。若弹出绿色成功提示，则配置正确。
...
2.  **保持版本更新**：
    *   在 GitHub 仓库页面点击 **Sync fork** -> **Update branch**，同步原作者的最早代码。
    *   同步后 Netlify 会自动触发重新构建，确保你拥有最新的功能（如自动同步开关）和 Bug 修复。

---

## 注意事项

*   **浏览器兼容性**：Safari 浏览器在处理长列表滚动时存在 WebKit 引擎兼容性问题，容易出现“滑动跳跃”或记录显示不全。**强烈建议在 macOS/Windows 上使用 Chrome 或 Edge 浏览器**以获得最流畅的体验。
